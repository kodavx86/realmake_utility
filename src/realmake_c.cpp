 ...
 */
 ...
 //constructors
 realmake_c::realmake_c(realmake_base &rb) {
 ...
 int realmake_c::output_makefile() {
	...
	if(!comp.empty())
	 ...
	 comp = GCC;
	 tmp = "which " + GCC + " > /dev/null";
	 rc = system(tmp.c_str());
	}

 	if(0 != rc) {
 	 std::cerr << "realmake error: cannot determine what C compiler to use" << std::endl;
	 std::cerr << "use the -c option to specify a compiler (i.e. -c /usr/local/gcc)" << std::endl;
	...
	while(NULL != (d_entry = readdir(dir))) {
	 if(DT_REG == d_entry->d_type) {
	  ...
	  if((buf.find_last_of(".") + 1) < buf.length()) {
	   if(0 == strcasecmp("c",buf.substr(buf.find_last_of(".") + 1).c_str()))
